...
class ControllerExtensionModuleSmsnot extends Controller {
	...
	private function set_callback($api_id = '', $url, $action) {
		if (extension_loaded('curl')) {
			$ch = curl_init("https://smstest.43vp.ru/callback/".$action);
			curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
			curl_setopt($ch, CURLOPT_TIMEOUT, 30);
			curl_setopt($ch, CURLOPT_POSTFIELDS, array(
				"api_id" => $api_id,
			...
			$response = file_get_contents('https://smstest.43vp.ru/callback/'.$action.'?api_id='.$api_id.'&url='.urlencode($url).'&json=1');
		}
